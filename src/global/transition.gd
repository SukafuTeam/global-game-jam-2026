...
func transition(new_scene: String):
	...
	tween.tween_property(left_wall, "position:x", left_wall_values.y, 0.5).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_EXPO)
	tween.parallel().tween_property(right_wall, "position:x", right_wall_values.y, 0.5).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_EXPO)
